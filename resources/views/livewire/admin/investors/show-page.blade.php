<div>
    <div class="flex items-start justify-between mb-6">
        <div class="flex items-center gap-4">
            @if ($investor->logo_path)
                <img src="{{ Storage::disk('public')->url($investor->logo_path) }}" class="h-12 w-12 rounded object-cover" alt="">
            @endif
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    @if ($investor->contact_phone) &middot; {{ $investor->contact_phone }} @endif
                </p>
            </div>
        </div>

        <div class="flex items-center gap-3">
            <a href="{{ route('admin.investors.edit', $investor) }}" wire:navigate>
                <x-secondary-button type="button">{{ __('Uredi') }}</x-secondary-button>
            </a>
            <a href="{{ route('admin.projects.create', ['investor' => $investor->id]) }}" wire:navigate>
                ...
                @forelse ($projects as $project)
                    <tr wire:key="project-{{ $project->id }}">
                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                            <a href="{{ route('admin.projects.show', $project) }}" wire:navigate class="font-medium hover:underline">
                                {{ $project->name }}
                            </a>
                        ...
                        <td class="px-6 py-4 text-end text-sm space-x-3 whitespace-nowrap">
                            <a href="{{ route('admin.projects.edit', $project) }}" wire:navigate class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Uredi') }}</a>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                            {{ __('Ovaj investitor još nema projekata.') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
