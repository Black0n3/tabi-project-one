<div>
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">
        <a href="{{ route('admin.investors.show', $unit->building->project->investor) }}" wire:navigate class="hover:underline">{{ $unit->building->project->investor->company_name }}</a>
        /
        <a href="{{ route('admin.projects.show', $unit->building->project) }}" wire:navigate class="hover:underline">{{ $unit->building->project->name }}</a>
        /
        <a href="{{ route('admin.buildings.show', $unit->building) }}" wire:navigate class="hover:underline">{{ $unit->building->name }}</a>
    </p>
    ...
    <div class="flex items-start justify-between mb-6">
        ...
        <a href="{{ route('admin.units.edit', $unit) }}" wire:navigate>
    ...
    <div class="flex items-center justify-between mb-3">
        ...
        <a href="{{ route('admin.rooms.create', ['unit' => $unit->id]) }}" wire:navigate class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
            + {{ __('Nova prostorija') }}
        </a>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg divide-y divide-gray-200 dark:divide-gray-700 max-w-2xl">
        @forelse ($rooms as $room)
            <div wire:key="room-{{ $room->id }}" class="px-4 py-3 flex items-center justify-between text-sm">
                ...
                <div class="space-x-3">
                    <a href="{{ route('admin.rooms.edit', $room) }}" wire:navigate class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Uredi') }}</a>
