<div>
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">
        <a href="{{ route('admin.investors.show', $project->investor) }}" wire:navigate class="hover:underline">{{ $project->investor->company_name }}</a>
    </p>
    ...
    <div class="flex items-start justify-between mb-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.buildings.create', ['project' => $project->id]) }}" wire:navigate>
                ...
                @forelse ($buildings as $building)
                    <tr wire:key="building-{{ $building->id }}">
                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                            <a href="{{ route('admin.buildings.show', $building) }}" wire:navigate class="font-medium hover:underline">
                                {{ $building->name }}
                            </a>
                        </td>
                        ...
                        <td class="px-6 py-4 text-end text-sm space-x-3 whitespace-nowrap">
                            <a href="{{ route('admin.buildings.edit', $building) }}" wire:navigate class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Uredi') }}</a>
