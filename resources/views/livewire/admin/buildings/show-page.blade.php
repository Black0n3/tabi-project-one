<div>
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">
        <a href="{{ route('admin.investors.show', $building->project->investor) }}" wire:navigate class="hover:underline">{{ $building->project->investor->company_name }}</a>
        /
        <a href="{{ route('admin.projects.show', $building->project) }}" wire:navigate class="hover:underline">{{ $building->project->name }}</a>
    </p>
    ...
    <div class="flex items-start justify-between mb-6">
        ...
        <a href="{{ route('admin.buildings.edit', $building) }}" wire:navigate>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section>
            <div class="flex items-center justify-between mb-3">
                ...
                <a href="{{ route('admin.floors.create', ['building' => $building->id]) }}" wire:navigate class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
                    + {{ __('Novi kat') }}
                </a>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($floors as $floor)
                    <div wire:key="floor-{{ $floor->id }}" class="px-4 py-3 flex items-center justify-between text-sm">
                        ...
                        <div class="space-x-3">
                            <a href="{{ route('admin.floors.edit', $floor) }}" wire:navigate class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Uredi') }}</a>
        ...
        <section>
            <div class="flex items-center justify-between mb-3">
                ...
                <a href="{{ route('admin.units.create', ['building' => $building->id]) }}" wire:navigate class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
                    + {{ __('Nova jedinica') }}
                </a>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($units as $unit)
                    <div wire:key="unit-{{ $unit->id }}" class="px-4 py-3 flex items-center justify-between text-sm">
                        <div>
                            <a href="{{ route('admin.units.show', $unit) }}" wire:navigate class="font-medium text-gray-900 dark:text-gray-100 hover:underline">{{ $unit->code }}</a>
                        ...
                        <div class="space-x-3">
                            <a href="{{ route('admin.units.edit', $unit) }}" wire:navigate class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Uredi') }}</a>
